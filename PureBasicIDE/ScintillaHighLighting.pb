...
CompilerIf #CompileWindows | #CompileLinux | #CompileMac
  ...
  Procedure HilightCallback(*StringStart.Ascii, Length, *Color, IsBold, TextChanged)
    ...
    ; replace the text only if it was changed by the highlightingengine (case correction)
    ;
    If TextChanged
  ...
  Procedure UpdateHighLighting()   ; hilight everything after a prefs update
    ...
    If EnableColoring Or EnableCaseCorrection
      ...
      If *ActiveSource\IsCode
        ...
        *HilightBuffer = *InBuffer
        ...
        HighLightingEngine(*InBuffer, InBufferLength, SendEditorMessage(#SCI_GETCURRENTPOS, 0, 0), @HilightCallback(), 1)
          ...
          *ActiveSource\LineHistory[0] = *ActiveSource\CurrentLine ; the 0 index is always the last seen line
          ...
          If *ActiveSource\ModifiedSinceUpdate = 1 And *ActiveSource\IsCode
            ...
            If *ActiveSource\CurrentLine <> *ActiveSource\CurrentLineOld And *ActiveSource\CurrentLineOld > 0
              ...
              ; update highlighting of the old line, as now keywords are not hilighted with the cursor inside (so do it when the mouse is moved away)
              ;
              If (EnableColoring Or EnableCaseCorrection) And OldLine$ <> ""
                ...
                ;
                Modified = GetSourceModified()  ; because the case correction changes the modified state!
                HighLightingEngine(*HilightBuffer, MemoryAsciiLength(*HilightBuffer), -1, @HilightCallback(), 1)
                SetSourceModified(Modified)
            ...
            *Buffer = AllocateMemory(range\chrg\cpMax - range\chrg\cpMin + 1)
            If *Buffer
              ...
              *HilightBuffer = *Buffer
              ...
              HighLightingEngine(*Buffer, reallength, currentPos-range\chrg\cpMin , @HilightCallback(), 1)
              SetSourceModified(Modified)
